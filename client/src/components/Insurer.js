...
const Insurer = ({mediWox, account, ethValue}) => {
    ...
    const getInsurerData = async () => {
        var insurer = await mediWox.methods.insurerInfo(account).call();
        setInsurer(insurer);
    }
    const getPolicyList = async () => {
        var pol = await mediWox.methods.getInsurerPolicyList(account).call();
        setPolicyList(pol)
    }
    const createPolicy = (e) => {
        e.preventDefault()
        mediWox.methods.createPolicy(polName, polCoverValue, polDuration, polPremium).send({from: account}).on('transactionHash', (hash) => {
    ...
    const getPatientList = async () => {
        var pat = await mediWox.methods.getInsurerPatientList(account).call();
        let pt = [];
    ...
    //     var claimsIdList = await mediWox.methods.getInsurerClaims(account).call();
    //     let cl = [];
    //     for(let i=claimsIdList.length-1; i>=0; i--){
    //         let claim = await mediWox.methods.claims(claimsIdList[i]).call();
    //         let patient = await mediWox.methods.patientInfo(claim.patient).call();
    //         let doctor = await mediWox.methods.doctorInfo(claim.doctor).call();
    ...
    //     mediWox.methods.approveClaimsByInsurer(claim.id).send({from: account, value: Web3.utils.toWei(value.toString(), 'Ether')}).on('transactionHash', (hash) => {
    //         return window.location.href = '/login'
    //     })
    // }
    ...
    const approveClaim = async (e, claim) => {
        e.preventDefault();
        console.log("Claim value:", claim.valueClaimed);
        console.log("ETH value:", ethValue);
        let value = claim.valueClaimed / ethValue;
        value = parseFloat(value.toFixed(18));
        console.log("Transaction value in ETH:", value);
        try {
            await mediWox.methods.approveClaimsByInsurer(claim.id).send({
                from: account,
    ...
    const rejectClaim = async (e, claim) => {
        mediWox.methods.rejectClaimsByInsurer(claim.id).send({from: account}).on('transactionHash', (hash) => {
            return window.location.href = '/login'
        })
    }
